...
"""
  ...
  Description:           Big list of countries/regions and associated adjectives
...
class Countries:
    """Provides fast lookup for country/region names and adjectives."""

    @classmethod
    def lookup(cls, name: str, adjectives: bool = True) -> bool:
        """Return True if name matches a country/region name (and optionally
        country/region adjectives)
        """
        name = name.lower()

        return name in cls.names or (adjectives and name in cls.adjectives)

    # From here (with minor changes)
    # https://en.wikipedia.org/wiki/List_of_adjectival_and_demonymic_forms_for_countries_and_nations
    _data = [
        # Taiwan is called "Chinese Taipei" for international events
    ...
    names = frozenset(c[0].lower() for c in _data)
    adjectives = frozenset(
        map(lambda s: s.lower(), chain.from_iterable(c[1] for c in _data))
    )
